...
console.log('🔧 在 Render 上設置 Bryanho 管理員帳戶...\n');

// 數據庫路徑 - 在 Render 上使用環境變量或默認路徑
const dbPath = process.env.DATABASE_PATH || path.join(__dirname, 'database', 'users.db');
    ...
    try {
        ...
        db.get('SELECT id, role FROM users WHERE username = ?', ['Bryanho'], async (err, row) => {
            ...
            db.run(sql, [
                ...
                console.log('   - 預測頁面: /index.html');
                console.log('\n📋 設置完成！您現在可以:');
                console.log('1. 訪問您的 Render 應用 URL');
